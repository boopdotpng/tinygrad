...
def hand_spec_kernel_cuda():
  # UOps map to PTX as follows:
  # - UOp.special("gidx*"/"lidx*") -> %ctaid.%tid registers
  # - AddrSpace.LOCAL -> .shared memory
  # - AddrSpace.REG -> registers
  # - UOp.barrier -> bar.sync 0
  # - ALU ops -> PTX ALU (fma.rn.f32 for float MULACC)
  ...
  As = UOp.placeholder((BLOCK_K, BLOCK_M), dtypes.float, slot=0, addrspace=AddrSpace.LOCAL)
...
if __name__ == "__main__":
  test_matmul(hand_spec_kernel_cuda(), N=N)
